                ...
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">Facility List</h4>
                            <div class="flex-shrink-0">
                                <a href="{{url('admin/facility/create')}}" class="btn btn-info">Add Facility</a>
                            </div>
                        </div><!-- end card header -->
                                ...
                                <div class="table-responsive pb-4">
                                    <table class="table align-middle table-nowrap mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            @forelse($facilities as $key=>$row)
                                            <tr row_id="{{$row->id}}" class="get_row_{{$row->id}}">
                                                <td>{{$key+1+($facilities->currentPage()-1) * ($facilities->perPage())}}</td>
                                                <td>{{$row->title ?? ''}}</td>
                                                <td> <a class="btn btn-info btn-sm waves-effect waves-info" href="{{url('admin/user?facility='.$row->id)}}">
                                                        <i class="fe fe-pencil"></i>{{$row->title ?? ''}} - Users
                                                    </a></td>
                                                ...
                                                <td>
                                                    <a class="btn btn-info btn-sm waves-effect waves-light" href="{{url('admin/facility/edit/'.$row->id)}}">
