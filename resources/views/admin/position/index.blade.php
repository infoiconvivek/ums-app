                    ...
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            ...
                            <div class="flex-shrink-0">
                                <a href="{{url('admin/position/create')}}" class="btn btn-info">Add Position</a>
                            </div>
                        </div><!-- end card header -->
                                ...
                                <div class="table-responsive pb-4">
                                    <table class="table align-middle table-nowrap mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            @forelse($positions as $key=>$row)
                                            <tr row_id="{{$row->id}}" class="get_row_{{$row->id}}">
                                                <td>{{$key+1+($positions->currentPage()-1) * ($positions->perPage())}}</td>
                                                <td>{{$row->title ?? ''}}</td>
                                                <td> <a class="btn btn-info btn-sm waves-effect waves-info" href="{{url('admin/user?position='.$row->id)}}">
                                                ...
                                                <td>
                                                    <a class="btn btn-info btn-sm waves-effect waves-light" href="{{url('admin/position/edit/'.$row->id)}}">
