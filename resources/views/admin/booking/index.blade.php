                                ...
                                <div class="table-responsive pb-4">
                                    <table class="table align-middle table-nowrap mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            @forelse($user_bookings as $key=>$row)
                                            <tr row_id="{{$row->id}}" class="get_row_{{$row->id}}">
                                                <td>{{$key+1+($user_bookings->currentPage()-1) * ($user_bookings->perPage())}}</td>
                                                <td>{{$row->first_name ?? ''}} {{$row->last_name ?? ''}}</td>
                                                <td>{{$row->email ?? ''}}</td>
                                                <td>{{$row->phone ?? ''}}</td>
                                                <td>{{$row->title ?? ''}}</td>
                                                <td>
                                                    <a class="btn btn-info btn-sm waves-effect waves-light" href="{{url('admin/booking/view/'.$row->booking_id)}}">
